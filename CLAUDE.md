...
### Net Worth
- `/networth` (⌘6): hero total + 30-day delta over an area chart (with assets/liabilities sub-line when debt exists), accounts split into Assets and Liabilities sections (divider rows, type icon chips, Synced/Stale/Sync-error badges, hover-reveal ↑/↓ reorder within section + edit), rail breakdown by account type (liabilities shown negative) + SimpleFIN sync card
